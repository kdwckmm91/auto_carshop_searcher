...
- **データ品質フィルタ**: 支払総額未取得・年式範囲外の車両を除外
- **割安性スコア**: 1万円あたりの残存価値を自動計算してスプレッドシートに記録
...
| 支払総額(万円) | ※ null の場合は除外されます |
| 本体価格(万円) | |
| 年式 | ※ `config.scraper_config.yearMin` より古い場合は除外されます |
| 走行距離(万km) | |
| 車検期限 | |
| 修復歴 | |
| 店舗名 | |
...
| URL | 車両詳細ページ |
| 取得日時 | JST (例: 2026-03-11 23:06:28) |
| スコア | バリュー効率スコア（1万円あたりの残存価値） |

### スコア計算式

$$Score = \frac{(15 - \text{走行距離[\text{万km}]}) + \max(0,\ 15 - (\text{現在年} - \text{年式}))}{\text{支払総額[\text{万円}]}}$$

- 期待寿命: 15万km、期待耐用年数: 15年
- **0.18以上**: 割安、**0.14以下**: 割高
